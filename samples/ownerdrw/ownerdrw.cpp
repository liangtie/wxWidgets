...
bool OwnerDrawnApp::OnInit(void)
{
    ...
    NEW_DEBUG OwnerDrawnFrame(NULL, "wxWidgets Ownerdraw Sample", 50, 50, 450, 340);
...
void OwnerDrawnFrame::InitMenu()
{
    ...
    wxMenu *sub_menu  = NEW_DEBUG wxMenu;
    ...
    // construct submenu
    pItem = NEW_DEBUG wxMenuItem(sub_menu, Menu_Sub1, "Submenu &first", "large");

    pItem->SetFont(fontLarge);
    sub_menu->Append(pItem);

    pItem = NEW_DEBUG wxMenuItem(sub_menu, Menu_Sub2, "Submenu &second", "italic",
                           wxITEM_CHECK);
    pItem->SetFont(fontItalic);
    sub_menu->Append(pItem);

    pItem = NEW_DEBUG wxMenuItem(sub_menu, Menu_Sub3, "Submenu &third", "underlined",
                           wxITEM_CHECK);
    pItem->SetFont(fontUlined);
    sub_menu->Append(pItem);

    // construct menu
    pItem = NEW_DEBUG wxMenuItem(file_menu, Menu_Test1, "&Uncheckable", "red item");
    pItem->SetFont(*wxITALIC_FONT);
    pItem->SetTextColour(wxColor(255, 0, 0));
    file_menu->Append(pItem);

    pItem = NEW_DEBUG wxMenuItem(file_menu, Menu_Test2, "&Checkable",
                           "checkable item", wxITEM_CHECK);
    pItem->SetFont(*wxSMALL_FONT);
    file_menu->Append(pItem);
    file_menu->Check(Menu_Test2, true);

    pItem = NEW_DEBUG wxMenuItem(file_menu, Menu_Test3, "&Disabled", "disabled item");
    ...
    pItem = NEW_DEBUG wxMenuItem(file_menu, Menu_Bitmap, "&Bell",
                           "check/uncheck me!", wxITEM_CHECK);
    pItem->SetFont(fontBmp);
    pItem->SetBitmaps(bmpBell);
    file_menu->Append(pItem);

    pItem = NEW_DEBUG wxMenuItem(file_menu, Menu_Bitmap2, "So&und",
    ...
    pItem = NEW_DEBUG wxMenuItem(file_menu, Menu_Submenu, "&Sub menu", "",
                           wxITEM_CHECK, sub_menu);
    pItem->SetFont(*wxSWISS_FONT);
    file_menu->Append(pItem);

    file_menu->AppendSeparator();
    pItem = NEW_DEBUG wxMenuItem(file_menu, Menu_Toggle, "&Disable/Enable\tCtrl+D",
                          "enables/disables the About-Item", wxITEM_NORMAL);
    pItem->SetFont(*wxNORMAL_FONT);
    file_menu->Append(pItem);

    // Of course Ctrl+RatherLongAccel will not work in this example:
    pAboutItem = NEW_DEBUG wxMenuItem(file_menu, Menu_About, "&About\tCtrl+RatherLongAccel",
    ...
    pItem = NEW_DEBUG wxMenuItem(file_menu, Menu_Quit, "&Quit", "Normal item",
                           wxITEM_NORMAL);
    file_menu->Append(pItem);

    wxMenu* drawn_menu = NEW_DEBUG wxMenu;
    pItem = NEW_DEBUG wxMenuItem(drawn_menu, Menu_Drawn1, "&Menu item\tCtrl+K");
    drawn_menu->Append(pItem);

    drawn_menu->AppendSeparator();

    pItem = NEW_DEBUG wxMenuItem(drawn_menu, Menu_Drawn2, "&Checked item",
        "check/uncheck me!", wxITEM_CHECK);
    drawn_menu->Append(pItem);
    drawn_menu->Check(Menu_Drawn2, true);

    pItem = NEW_DEBUG wxMenuItem(drawn_menu, Menu_Drawn3, "&Radio item",
        "check/uncheck me!", wxITEM_RADIO);
    drawn_menu->Append(pItem);

    drawn_menu->AppendSeparator();

    pItem = NEW_DEBUG wxMenuItem(drawn_menu, Menu_Drawn4, "&Disabled item\tCtrl+RatherLongAccel",
        "disabled item");
    pItem->Enable(false);
    drawn_menu->Append(pItem);

    pItem = NEW_DEBUG wxMenuItem(drawn_menu, Menu_Drawn5, "&Other\tCtrl+O", "other item");
    pItem->SetTextColour(*wxRED);
    drawn_menu->Append(pItem);

    wxMenu* native_menu = NEW_DEBUG wxMenu;
    pItem = NEW_DEBUG wxMenuItem(native_menu, Menu_Native1, "&Menu item\tCtrl+K");
    native_menu->Append(pItem);

    native_menu->AppendSeparator();

    pItem = NEW_DEBUG wxMenuItem(native_menu, Menu_Native2, "&Checked item",
        "check/uncheck me!", wxITEM_CHECK);
    native_menu->Append(pItem);
    native_menu->Check(Menu_Native2, true);

    pItem = NEW_DEBUG wxMenuItem(native_menu, Menu_Native3, "&Radio item",
        "check/uncheck me!", wxITEM_RADIO);
    native_menu->Append(pItem);

    native_menu->AppendSeparator();

    pItem = NEW_DEBUG wxMenuItem(native_menu, Menu_Native4, "&Disabled item\tCtrl+RatherLongAccel",
        "disabled item");
    pItem->Enable(false);
    native_menu->Append(pItem);

    pItem = NEW_DEBUG wxMenuItem(native_menu, Menu_Native5, "&Other\tCtrl+O", "other item");
    native_menu->Append(pItem);
         ...
         : wxFrame(frame, wxID_ANY, title, wxPoint(x, y), wxSize(w, h))
{
    ...
    wxPanel *pPanel = NEW_DEBUG wxPanel(this);
    ...
    wxString *astrChoices = NEW_DEBUG wxString[WXSIZEOF(aszChoices)];
    unsigned int ui;
    for ( ui = 0; ui < WXSIZEOF(aszChoices); ui++ )
        astrChoices[ui] = aszChoices[ui];

    m_pListBox = NEW_DEBUG wxCheckListBox
        (
            pPanel,             // parent
    ...
    astrChoices = NEW_DEBUG wxString[WXSIZEOF(aszColors)];
    ...
    wxListBox *pListBox = NEW_DEBUG wxListBox
        (
            pPanel,              // parent
