...
@li @b Portability: It is a common practice to use @e printf() statements or
    ...
    @code
    wxLog* logger = NEW_DEBUG wxLogStream(&cout);
...
To create a NEW_DEBUG log target class you only need to derive it from wxLog and
...
helpful to see how you can create a NEW_DEBUG log target class and, of course, may
...
without writing a NEW_DEBUG log target class (which, aside from being a matter of
